...
class Slogix
{
    public static function evaluate($slogix, $sets)
    {
        ...
        {								
            if(count($spaces)==0)
            {
                if(gettype($operand) === "string")
                    ...
                    $set = self::evaluate($operand, $sets);
    ...
    public static function decode($slogix, &$i=0, &$ast = array())
    {								
        ...
        switch($operator)
        {
            ...
            case "or":
                ...
                $operands = self::get_operands($slice);
    ...
    private static function get_operands($input)
    {
        $result = array();
            ...
            {																
                ...
                {																				
                    ...
                    {																								
                        ...
                        $balanced = self::is_balanced($sub_slogix);
                        
                        if($balanced)
                        {
                            array_push($result, self::decode($sub_slogix));
                            break;
                        }
                        else
                            $sub_exp_len++;
    ...
    private static function is_balanced($slogix)
    {								
